...
var AssetsManifest2 = class {
  ...
  async get(pathname) {
    let pathHash = await hashPath2(pathname), entry = binarySearch2(this.data, pathHash);
    return entry ? Uint8ToHexString(entry) : null;
  }
}, hashPath2 = async (path) => {
  let data = new TextEncoder().encode(path), hashBuffer = await crypto.subtle.digest(
    "SHA-256",
    data.buffer
  );
  return new Uint8Array(hashBuffer, 0, 16);
}, binarySearch2 = (manifest, pathHash) => {
...
function comparePathHashWithEntry(searchValue, manifest, entryIndex) {
  let entryOffset = 20 + entryIndex * 40;
  for (let offset = 0; offset < 16; offset++, entryOffset++) {
    let s = searchValue[offset], e = manifest[entryOffset];
    if (s < e)
      return -1;
    if (s > e)
      return 1;
...
var worker_default = class extends WorkerEntrypoint {
  ...
  async unstable_exists(pathname, _request) {
    let binarySearchVersion = Math.random() < 0.5 ? "current" : "perfTest", analytics = new ExperimentAnalytics(this.env.EXPERIMENT_ANALYTICS), performance = new PerformanceTimer(this.env.UNSAFE_PERFORMANCE);
    return (this.env.JAEGER ?? mockJaegerBinding()).enterSpan("unstable_exists", async (span) => {
      ...
      try {
        ...
        if (binarySearchVersion === "perfTest")
          try {
            eTag = await new AssetsManifest2(
              this.env.ASSETS_MANIFEST
            ).get(pathname);
          } catch {
            analytics.setData({ binarySearchVersion: "current-fallback" }), eTag = await new AssetsManifest(this.env.ASSETS_MANIFEST).get(pathname);
          }
        else
          eTag = await new AssetsManifest(this.env.ASSETS_MANIFEST).get(pathname);
        return span.setTags({
          path: pathname,
          found: eTag !== null,
          etag: eTag ?? ""
