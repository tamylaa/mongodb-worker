...
async function handleHealth(d1Client2, env) {
  const envInfo = {
    ...
    D1_CLIENT_TYPE: d1Client2 ? "initialized" : "not initialized",
    D1_CLIENT_DB_TYPE: d1Client2 && d1Client2.db ? typeof d1Client2.db : "no db",
    TIMESTAMP: (/* @__PURE__ */ new Date()).toISOString()
  };
  let dbStatus = "unknown";
  try {
    if (d1Client2 && d1Client2.db) {
      const result = await d1Client2.db.prepare("SELECT 1 as test").first();
